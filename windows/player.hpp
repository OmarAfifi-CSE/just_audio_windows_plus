...
public:
  ...
  JustAudioEventSink::JustAudioEventSink(flutter::BinaryMessenger* messenger, const std::string& id) {
    ...
    auto event_handler = std::make_unique<flutter::StreamHandlerFunctions<>>(
      ...
      std::lock_guard<std::mutex> lock(self->sink_mutex_);
...
private:
  ...
  std::recursive_mutex player_mutex_;
  ...
  void Dispose() {
    ...
    {
      std::lock_guard<std::recursive_mutex> lock(player_mutex_);
      event_sink_.reset();
      data_sink_.reset();
    }
  }
...
public:
  ...
  */
  ...
  void AudioPlayer::broadcastPlaybackEvent() {
    ...
    std::lock_guard<std::recursive_mutex> lock(player_mutex_);
    if (disposed_.load() || !event_sink_) return;
    ...
    try {
      ...
      if (mediaPlaybackList && mediaPlaybackList.Items().Size() > 0) {
  ...
  void AudioPlayer::broadcastDataEvent() {
    if (disposed_.load()) return;
    std::lock_guard<std::recursive_mutex> lock(player_mutex_);
    if (disposed_.load() || !data_sink_) return;
